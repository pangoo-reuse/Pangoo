//  This codes are generated automatically. Do not modifyActivity!
package com.os.pangoo.a.activity;

import android.os.Bundle;
import android.view.View;
import androidx.databinding.DataBindingUtil;
import com.os.lib.help.ExcludeView;
import com.os.pangoo.BaseActivity;
import com.os.pangoo.R;
import com.os.pangoo.databinding.AbcdeBinding;
import com.os.pangoo.vm.AbcdeViewModel;
import java.lang.Override;
import java.lang.System;

/**
 * this is autoExclude class ,please dont delete inject 'ExcludeView';
 */
@ExcludeView
public class Abcde extends BaseActivity {
  public AbcdeBinding abcdeBinding;

  @Override
  protected View ui(Bundle sa) {
    abcdeBinding = DataBindingUtil.setContentView(this, R.layout.abcde);
    AbcdeViewModel abcdeViewModel = new AbcdeViewModel();
    abcdeBinding.setViewModel(abcdeViewModel);
    return null;
  }

  @Override
  protected void statusBar(Bundle sa) {
    System.out.println("hello , Pangoo !");
  }
}
